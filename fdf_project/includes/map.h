...
void	split_point_token(const char *token, char **h_str, char **c_str, char **comma);
int		parse_point(const char *token, int *z, int *color, int default_color);
int		process_point(t_map *map, char *token, int x, int row_idx);
int		process_row_points(t_map *map, char **cols, int row_idx);

// map_read.c
void	strip_newline(char *line);
int		open_map_file(const char *filename);
int		read_map_lines(int fd, char **lines, int *line_count, int *col_count);
